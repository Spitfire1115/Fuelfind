...
function blinkBadge() {
  ...
  // Force browser to re-read the element before re-adding the class
  void badge.offsetWidth;

  badge.classList.add('badge-blink');

  // Remove class after animation finishes so it can blink again next time
  setTimeout(function() {
    badge.classList.remove('badge-blink');
  }, 1300);
}
...
window.onload = function() {
  initMapDefault();   // Shows map immediately
}
// Show a default map (Hyderabad) before GPS kicks in
